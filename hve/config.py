...
class HveConfig:
    ...
    db_path: Path = field(
        default_factory=lambda: _env_path("HVE_DB_PATH", _default_db_path())
    )
    knowledge_dir: Path = field(
        default_factory=lambda: _env_path(
            "HVE_KNOWLEDGE_DIR", _configured_home() / "knowledge"
        )
    )
    reports_dir: Path = field(
        default_factory=lambda: _env_path(
            "HVE_REPORTS_DIR", _configured_home() / "reports"
        )
    )
    backups_dir: Path = field(
        default_factory=lambda: _env_path(
            "HVE_BACKUPS_DIR", _configured_home() / "backups"
        )
    )
